...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            JalwaTheme {
                val vm: JalwaViewModel = viewModel()
                JalwaApp(vm)
            }
        }
    }
...
@Composable
fun JalwaApp(vm: JalwaViewModel) {
            ...
            if (canGoBack) {
                TopAppBar(
                    ...
                    navigationIcon = {
                        IconButton(onClick = {
                            when (state.screen) {
                                AppScreen.DETECTION, AppScreen.PERMISSIONS, AppScreen.ACCOUNT -> vm.show(AppScreen.HOME)
...
@Composable
private fun HomeScreen(vm: JalwaViewModel, user: UserEntity?) {
...
private fun DetectionScreen(vm: JalwaViewModel, state: UiState) {
    Column(Modifier.fillMaxSize().padding(22.dp)) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedButton(onClick = { vm.pauseDetection() }, Modifier.weight(1f)) { Text("Pause") }
...
private fun TemplatesScreen(vm: JalwaViewModel, templates: List<TemplateEntity>) {
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        runCatching {
            ...
            vm.importTemplate(filename, pendingName, pendingThreshold)
            name = ""
        }.onFailure {
            // The ViewModel keeps user-facing errors generic; no stack traces are shown.
        }
    }
    Column(Modifier.fillMaxSize().padding(22.dp)) {
