...
The repository was consulted as an openly licensed reference for the requested testing-utility domain. ATPILOT's authentication, administrator system, subscriptions, private template storage, Compose UI, MediaProjection pipeline, overlay controller, and detection state machine are newly implemented and are not a renamed copy of Smart AutoClicker.
...
The complete license text is included in `third-party-licenses/GPL-3.0.txt`.
